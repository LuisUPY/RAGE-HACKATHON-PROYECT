"""Configurable company chatbot using RAGE v2 UserGate."""
...
class ProfileChatResultV2:
  ...
  def total_ms(self) -> float:
    return self.rage_ms + self.assistant_ms


@dataclass
class ProfileChatbotV2:
  """Product shell with v2 UserGate — ALERT does not block chat."""
  ...
  def __post_init__(self) -> None:
    self.gate = UserGate(self.profile)

  def handle_turn(self, user_text: str, *, offline: bool = False) -> ProfileChatResultV2:
    result = self.gate.evaluate(user_text)
    if result.blocked:
      msg = result.block_message
      self.gate.record_assistant(msg)
      return ProfileChatResultV2(
        user_text=user_text,
        gate=result,
        assistant_text=msg,
        assistant_ms=0.0,
