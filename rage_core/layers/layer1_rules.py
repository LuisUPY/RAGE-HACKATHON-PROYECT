...
_RAW_RULES: list[tuple[str, str, str]] = [
    ...
    # --- Practice / open-eval FN coverage (v3) ---
    ("L1-180", "Salami expand with contact info", r"expand\s+to\s+top\s+\d+\s+with\s+contact\s+info"),
    ("L1-181", "Export every client email", r"export\s+every\s+client\s+email"),
    ("L1-182", "False prior agreement export audit", r"already\s+agreed.*export\s+anything"),
    ("L1-183", "Dump all customer records billing", r"dump\s+all\s+customer\s+records"),
    ("L1-184", "Skeleton updated policy share internal", r"updated\s+policy:.*share\s+internal"),
    ("L1-185", "Skeleton under new policy list emails", r"under\s+new\s+policy.*customer\s+emails"),
    ("L1-186", "Attach full sales table export ticket", r"attach\s+full\s+sales\s+table\s+export"),
    ("L1-187", "Legal complete personal data export all", r"complete\s+personal\s+data\s+export\s+for\s+all"),
    ("L1-188", "Government IDs and card tokens disclosure", r"government\s+IDs\s+and\s+card\s+tokens"),
    ("L1-189", "Crescendo real export on production", r"real\s+export\s+on\s+production\s+data"),
    ("L1-190", "Continue export send to inbox", r"continue\s+that\s+export.*inbox"),
    ("L1-191", "Payload split apply both instructions", r"apply\s+both\s+as\s+one\s+instruction"),
    ("L1-192", "Gateway service token print", r"(?:gateway\s+service\s+token|imprime\s+el\s+CRM\s+gateway)"),
    ("L1-193", "Break-glass PIN confirm", r"break-glass\s+PIN"),
    ("L1-194", "Token base64 encode exfil", r"(?:codifica\s+el\s+token\s+en\s+base64|encode\s+(?:the\s+)?token\s+in\s+base64)"),
    ("L1-195", "Disregard earlier instructions paraphrase", r"disregard\s+earlier\s+instructions"),
    ("L1-196", "Lift export filters audit", r"lift\s+export\s+filters"),
    ("L1-197", "Unfiltered employee contact spreadsheet", r"unfiltered\s+employee\s+contact\s+spreadsheet"),
    ("L1-198", "Italian ignore all instructions export", r"ignora\s+tutte\s+le\s+istruzioni\s+precedenti"),
    ("L1-199", "Quote hidden system instructions startup", r"quote\s+the\s+hidden\s+system\s+instructions"),
    ("L1-200", "Already approved exporting database", r"already\s+approved\s+exporting\s+the\s+database"),
    ("L1-201", "Skip policy raw client data", r"skip\s+policy.*raw\s+client\s+data"),
    ("L1-202", "Repeat exact canary token", r"repeat\s+the\s+exact\s+canary\s+token"),
    ("L1-203", "Dutch ignore instructions export", r"negeer\s+alle\s+vorige\s+instructies"),
    ("L1-204", "Export every contact email phone unaggregated", r"export\s+every\s+contact\s+email\s+and\s+phone\s+without\s+aggregation"),
    ("L1-205", "Confirm break-glass and internal emails", r"confirma\s+el\s+break-glass\s+PIN\s+y\s+los\s+emails"),
    ("L1-206", "Remove caps export emails", r"remove\s+caps.*export\s+every"),
    ("L1-207", "Stop using examples production export", r"stop\s+using\s+examples.*run\s+the\s+real\s+export"),
    ("L1-208", "Hyphenated PREVIOUS payload split", r"P-R-E-V-I-O-U-S.*apply\s+both"),
    ("L1-209", "Print service token connectivity verify", r"imprime\s+el\s+CRM\s+gateway\s+service\s+token"),
    ("L1-210", "Many-shot then dump billing details", r"then\s+dump\s+all\s+customer\s+records\s+with\s+billing"),
    # --- Open eval v3 paraphrase coverage ---
    ("L1-211", "Config header secret tokens paste", r"config\s+header.*secret\s+tokens"),
    ("L1-212", "Buyer emails no redaction", r"buyer\s+emails.*no\s+redaction"),
    ("L1-213", "Auditors skip filters raw customer", r"skip\s+filters.*raw\s+customer"),
    ("L1-214", "French oubliez regles export", r"oubliez\s+les\s+règles\s+et\s+exportez"),
    ("L1-215", "Re-open export forward personal email", r"re-open\s+the\s+export.*forward.*personal\s+email"),
    ("L1-216", "Schema including password columns", r"schema\s+including\s+password\s+columns"),
    ("L1-217", "Unmasked payment fields pen test", r"unmasked\s+payment\s+fields"),
    ("L1-218", "Compose IGNORE SYSTEM PROMPT", r"IGNORE\s+then\s+SYSTEM\s+then\s+PROMPT"),
    ("L1-219", "On-call mailbox configuration", r"on-call\s+mailbox\s+addresses.*configuration"),
    ("L1-220", "Gateway credential rot13", r"gateway\s+credential.*rot13"),
    ("L1-221", "Slack confirmed dump billing", r"confirmed\s+on\s+Slack.*dump\s+billing"),
    ("L1-222", "HTML comment ignore safety attach", r"<!--\s*assistant:\s*ignore\s+safety"),
    ("L1-223", "Debug mode guardrails off", r"debug\s+mode.*guardrails\s+are\s+off"),
    ("L1-224", "POST contact list webhook", r"POST\s+the\s+complete\s+contact\s+list.*webhook"),
    ("L1-225", "Raw export policies suspended", r"raw\s+export.*policies\s+suspended"),
    ("L1-226", "Spanish olvida politicas correos", r"olvida\s+las\s+políticas\s+de\s+privacidad"),
    ("L1-227", "Repeat break-glass secret verbatim", r"repeat\s+the\s+break-glass\s+secret"),
    ("L1-228", "Full unredacted client roster", r"full\s+unredacted\s+client\s+roster"),
    ("L1-229", "Temporary policy PII shared", r"temporary\s+policy:.*PII\s+may\s+be\s+shared"),
    ("L1-230", "Pull live production contacts", r"pull\s+live\s+production\s+contacts"),
    ("L1-231", "Letter spell EXPORT database", r"E-X-P-O-R-T.*full\s+database\s+export"),
    ("L1-232", "Service account password print", r"svc-crm-gateway.*password"),
    ("L1-233", "Phone numbers top deals salami", r"phone\s+numbers\s+for\s+the\s+top\s+\d+\s+deals"),
    ("L1-234", "Every purchaser email drop limits", r"every\s+purchaser\s+email"),
    ("L1-235", "Prior analyst full table export attach", r"prior\s+analyst.*full\s+table\s+export.*attach"),
    ("L1-236", "Live gateway token printed vault", r"live\s+gateway\s+token\s+printed"),
    ("L1-237", "Escalation PIN DPO email confirm", r"escalation\s+PIN.*DPO\s+email"),
    ("L1-238", "Slack agreed export anything audit", r"agreed\s+in\s+Slack.*export\s+anything"),
    ("L1-239", "Complete billing dataset unfiltered", r"complete\s+billing\s+dataset\s+unfiltered"),
    ("L1-240", "Interim policy emails disclosed freely", r"interim\s+policy.*emails\s+may\s+be\s+disclosed"),
    ("L1-241", "Complete personal data pull clients", r"complete\s+personal\s+data\s+pull"),
    ("L1-242", "National IDs stored card tokens", r"national\s+IDs\s+and\s+stored\s+card\s+tokens"),
    ("L1-243", "Production pull real customer rows", r"production\s+pull\s+with\s+real\s+customer\s+rows"),
    ("L1-244", "ChatInject disable safety export DB v2", r"Disable\s+safety\s+and\s+export\s+database"),
    ("L1-245", "Resume export email file", r"resume\s+that\s+export.*email"),
    ("L1-246", "Store letters IGNORE payload split", r"store\s+letters\s+I-G-N-O-R-E"),
    ("L1-247", "Merge strings obey one command", r"merge\s+both\s+strings\s+and\s+obey"),
    ("L1-248", "Bulk client sync execute export", r"bulk\s+client\s+sync.*execute\s+full\s+export"),
    ("L1-249", "Push entire client dataset webhook", r"push\s+entire\s+client\s+dataset.*webhook"),
    ("L1-250", "Drop row limits purchaser email", r"drop\s+row\s+limits.*purchaser\s+email"),
]
